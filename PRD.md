...
> **核心理念**：Vector-Lake 系统不造轮子。Python 生态已有大量成熟开源项目可复用，覆盖文档解析、OCR、Chunking、Embedding、Audio 转录、SQL 查询、工作流编排、RAG 框架、对象存储、元数据管理 12 个层级。
>
> **设计哲学**：**不与开源项目竞争，只在其之上组装**。Lake 的核心价值在于"统一抽象 + Plugin 编排 + 元数据可靠性 + 多模态融合"，具体能力由最好的开源项目提供。
>
> 完整的详细复用清单（含代码示例、benchmark、License、踩坑指南）见 [PRD_REVIEW.md §十](file:///workspace/PRD_REVIEW.md) 开源项目复用清单。

### 20.1 分层复用总览（精选）

| 层级 | 子能力 | v0.1 默认 | 备选 |
|---|---|---|---|
| **L0 存储** | OSS / S3 API | [boto3](https://github.com/boto/boto3) + [PyArrow](https://github.com/apache/arrow) | [lakeFS](https://github.com/treeverse/lakeFS)（v0.2） |
| **L1 元数据** | Catalog / 事务日志 | （自研 §5.12） | [delta-rs](https://github.com/delta-io/delta-rs) / [PyIceberg](https://github.com/apache/iceberg-python)（v0.2） |
| **L2 文件解析** | PDF 解析 | [Docling](https://github.com/docling-project/docling)（IBM 工业级） | [MinerU](https://github.com/opendatalab/MinerU)（学术论文） / [Marker](https://github.com/datalab-to/marker)（快速） |
| **L2 文件解析** | 通用多格式 | [MarkItDown](https://github.com/microsoft/markitdown) | Docling（也支持） |
| **L3 OCR** | 多语言 OCR | [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR)（80+ 语言） | [Tesseract](https://github.com/tesseract-ocr/tesseract)（轻量） |
| **L4 Chunking** | 通用分块 | [Chonkie](https://github.com/chonkie-inc/chonkie)（10+ 策略） | [LangChain SemanticChunker](https://python.langchain.com) |
| **L5 Embedding** | Embedding 推理 | [Infinity](https://github.com/michaelfeil/infinity) | [TEI](https://github.com/huggingface/text-embeddings-inference) |
| **L5 Embedding** | 文本 | [BGE-M3](https://github.com/FlagOpen/FlagEmbedding) | [Qwen3-Embedding](https://huggingface.co/Qwen) |
| **L5 Embedding** | 多模态 | [Jina Embeddings v4](https://huggingface.co/jinaai) | [ColPali](https://github.com/illuin-tech/colpali) |
| **L6 音频转录** | ASR | [WhisperX](https://github.com/m-bain/whisperX)（含说话人分离） | [faster-whisper](https://github.com/SYSTRAN/faster-whisper) |
| **L7 SQL 查询** | 嵌入式 SQL | [DuckDB](https://github.com/duckdb/duckdb) | — |
| **L8 编排** | 任务队列 | [Celery](https://github.com/celery/celery) + Redis | [Dagster](https://github.com/dagster-io/dagster) / [Prefect](https://github.com/PrefectHQ/prefect) / [Temporal](https://github.com/temporalio/temporal) |
| **L9 RAG 参考** | 端到端 RAG | [OpenRAG](https://github.com/langflow-ai/openrag) / [RAGFlow](https://github.com/infiniflow/ragflow) | [Haystack](https://github.com/deepset-ai/haystack) |
| **L10 元数据可靠性** | 后台任务 | [APScheduler](https://github.com/agronholm/apscheduler) | [Celery Beat](https://github.com/celery/celery) |
| **L10 元数据可靠性** | 数据校验（v0.3） | [Great Expectations](https://github.com/great-expectations/great_expectations) | — |
| **L10 元数据可靠性** | 元数据 UI（v0.3） | [DataHub](https://github.com/datahub-project/datahub) | [Apache Atlas](https://github.com/apache/atlas) |
| **L11 API** | HTTP 框架 | [FastAPI](https://github.com/tiangolo/fastapi) | — |
| **L11 API** | 限流 | [slowapi](https://github.com/laurentS/slowapi) | — |

### 20.2 v0.1 起步技术栈（6 周跑通 MVP）
...
### 20.3 v0.2 演进（引入 Lake 标准）

| 引入 | 替代 | 价值 |
|---|---|---|
| [PyIceberg](https://github.com/apache/iceberg-python) | （自研）事务日志 | 复用 Iceberg ACID 社区生态 |
| [Dagster](https://github.com/dagster-io/dagster) | Celery | asset-centric 编排更适合 Entity 建模 |
| [Temporal](https://github.com/temporalio/temporal) | Celery（关键任务） | exactly-once + 故障恢复 |
| [lakeFS](https://github.com/treeverse/lakeFS) | 无 | 提供 Entity 零拷贝快照（git-like） |

### 20.4 v0.3 高级能力

| 引入 | 价值 |
|---|---|
| [ColPali](https://github.com/illuin-tech/colpali) | image-typed Entity 的多向量视觉检索 |
| [DataHub](https://github.com/datahub-project/datahub) | 运维侧 Entity 目录 + 血缘可视化 |
| [Great Expectations](https://github.com/great-expectations/great_expectations) | Rep↔Raw 一致性校验（与 Reconciler 互补） |
| [VectorDBBench](https://github.com/zilliztech/VectorDBBench) | Lance vs 其他向量库 benchmark |

### 20.5 Vector-Lake 必须自研的核心能力

虽然要最大化复用，但以下 8 项能力**必须自研**，因为没有开源项目完全匹配：
...
| 项目 | 定位 | Vector-Lake 关系 |
|---|---|---|
| [OpenRAG](https://github.com/langflow-ai/openrag) | 端到端 RAG 平台（Docling + OpenSearch + Langflow） | 借鉴架构，**替换 OpenSearch → Lance**、**增加 Wiki Projector**、**增加多消费者** |
| [RAGFlow](https://github.com/infiniflow/ragflow) | 开箱即用 RAG（DeepDoc + Infinity + Elasticsearch） | 借鉴 DeepDoc 思想，**核心用 Docling**、**存储用 Lance** |
| [Haystack](https://github.com/deepset-ai/haystack) | 生产级 RAG pipeline | 借鉴 pipeline 抽象，**改用 RepStep/IndexStep 解耦** |
| [LangChain](https://github.com/langchain-ai/langchain) | 通用 LLM 编排 | **不作为核心依赖**（太重），仅用其少量组件（如 SemanticChunker） |
| [LlamaIndex](https://github.com/run-llama/llama_index) | 数据摄取框架 | 借鉴其 150+ data connectors 设计思想 |
| [LakeFS](https://github.com/treeverse/lakeFS) | Git-like 数据湖 | v0.2 集成作为版本控制层 |
| [PyIceberg](https://github.com/apache/iceberg-python) | Iceberg Python API | v0.2 集成作为 Catalog 层 |
| [Lance](https://github.com/lancedb/lance) | 多模态 Lake 格式 | 核心存储（已选） |

### 20.7 选择开源项目的 5 个原则

1. **License 兼容性**：优先 Apache-2.0 / MIT；慎用 GPL（传染性）；避免 AGPL
2. **社区活跃度**：优先 6 个月内活跃；1K+ stars；有商业公司背书
3. **生产验证**：优先已被大公司使用；公开 benchmark；v1.0+ release
4. **可扩展性**：优先 Python 实现；提供 Python API + CLI；支持插件机制
5. **依赖轻量**：避免 1GB+ 安装大小；避免强依赖特定硬件
