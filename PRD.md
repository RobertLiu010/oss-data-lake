...
### 1.5 v0.1 核心路径：一切皆 MD

> **设计哲学**：v0.1 专注于 Markdown 作为唯一的知识表示格式。所有文档最终都变成 `canonical_md`，再基于 MD 构建 text 索引。这大幅简化了 v0.1 的 Pipeline 复杂度，同时覆盖了 80% 的知识检索场景。

**v0.1 的两条 Pipeline 路径**：

```text
路径 1：Markdown 直入
  .md 文件 → RepStep parse → canonical_md（直接复制/微调格式）→ IndexPipeline text

路径 2：办公文档 → PDF → VLM → MD
  .doc/.docx/.ppt/.pptx/.wps/.wpt/.dps/.dpt/.pdf
    → RepStep convert_to_pdf（LibreOffice）→ page_image
    → RepStep visual_recognize（VLM）→ canonical_md
    → IndexPipeline text
```
...
| --- | --- | --- |
| `.md` | 路径 1（直接 parse） | 无 |
| `.pdf` | 路径 2（VLM 提取） | VLM 服务 |
| `.doc` / `.docx` | 路径 2（→ PDF → VLM） | LibreOffice + VLM |
| `.ppt` / `.pptx` | 路径 2（→ PDF → VLM） | LibreOffice + VLM |
| `.wps` / `.wpt` / `.dps` / `.dpt` | 路径 2（→ PDF → VLM） | LibreOffice + VLM |
| `.txt` / `.html` / `.rtf` | 路径 1（parse 转 MD） | 无 / html2md |

**v0.1 不支持的格式（v0.2+）**：

| 格式 | 版本 | 说明 |
| --- | --- | --- |
| 图片（jpg/png/...） | v0.2 | Jina V5 Omni image embedding |
| 音频（wav/mp3/...） | v0.2 | Jina V5 Omni audio embedding |
| 视频（mp4/avi/...） | v0.2 | 关键帧抽取 + 音轨转写 |
| 表格（csv/xlsx/...） | v0.2 | DuckDB 结构化检索 |
| URL 爬取 | v0.2 | fetch_url + parse_html |

**v0.1 的 entity_type 简化**：

- v0.1 仅 `document` 一种 entity_type
- v0.2 扩展 `table` / `image` / `audio` / `video`

**v0.1 的 RepPipeline 简化**：

| Pipeline | v0.1 | v0.2 |
| --- | --- | --- |
| `rep_pipeline_a`（parse） | ✅ | ✅ |
| `rep_pipeline_b`（视觉识别：render_page → visual_recognize） | ✅ | ✅ |
| `rep_pipeline_d`（mind_map） | ❌ | ✅ |
| `rep_pipeline_e`（LLM 编译） | ❌ | ✅ |
| `rep_pipeline_f`（表格提取） | ❌ | ✅ |
| `rep_pipeline_g`（音频转写） | ❌ | ✅ |
| `rep_pipeline_h`（视频关键帧） | ❌ | ✅ |

**v0.1 的 IndexPipeline 简化**：

| Pipeline | v0.1 | v0.2 |
| --- | --- | --- |
| `index_pipeline_text`（semantic + lexical） | ✅ | ✅ |
| `index_pipeline_image` | ❌ | ✅ |
| `index_pipeline_video` | ❌ | ✅ |
| `index_pipeline_structural`（DuckDB） | ❌ | ✅ |
| `index_pipeline_graph` | ❌ | ✅ |

### 1.6 用户角色（User Personas）
...
### 1.7 竞争定位（Competitive Positioning）
