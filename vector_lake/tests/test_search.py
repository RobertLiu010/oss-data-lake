...
async def _index_test_chunks(
    ...
    """Helper: upsert chunks with mock vectors for a given entity (parquet only)."""
    chunks_with_vectors = []
...
async def _index_and_sync(
    ...
    """Helper: upsert chunks to parquet, then sync to LanceDB."""
    count = await _index_test_chunks(index_service, ws, col, entity_id, texts)
    await index_service.sync_to_lance(ws, col, entity_id)
    return count
...
class TestSearchSemantic:
    ...
    async def test_search_semantic(self, index_service: IndexService):
        ...
        await _index_and_sync(index_service, ws, col, eid, [
...
class TestCountEntityChunks:
    ...
    async def test_count_entity_chunks(self, index_service: IndexService):
        ...
        await _index_and_sync(index_service, ws, col, eid, texts)
...
class TestTableExists:
    ...
    async def test_table_exists(self, index_service: IndexService):
        ...
        await _index_and_sync(index_service, ws, col, eid, ["Hello world"])
...
class TestGetIndexedEntityIds:
    ...
    async def test_get_indexed_entity_ids(self, index_service: IndexService):
        ...
        await _index_and_sync(index_service, ws, col, "ent_a", ["Chunk A1", "Chunk A2"])
        await _index_and_sync(index_service, ws, col, "ent_b", ["Chunk B1"])
